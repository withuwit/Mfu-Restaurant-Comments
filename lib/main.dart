  ...
  Future<void> _fetchFavoriteIds() async {
    try {
      final snapshot = await firestore
          ...
          .get();
      setState(() {
        _favoriteIds = snapshot.docs.map((doc) => doc.data()['restaurant_id']?.toString() ?? '').where((id) => id.isNotEmpty).toSet();
      });
    } catch (e) { /* Silent */ }
  }
  ...
  void _applyFilters() {
    ...
    setState(() {
      _filteredRestaurants = _restaurants.where((r) {
        ...
        // Time filter
        final openHour = (r['time_open_hour'] as num?)?.toInt() ?? 0;
        final closeHour = (r['time_close_hour'] as num?)?.toInt() ?? 24;
        bool matchTime = true;
  ...
  Future<void> _fetchFavorites() async {
    try {
      ...
      for (var favDoc in favSnapshot.docs) {
        final restaurantId = favDoc.data()['restaurant_id']?.toString() ?? '';
